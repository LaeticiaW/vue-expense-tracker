            ...
            <v-card>
                ...
                <v-card-text class="dialog-content">
                    <v-form ref="form">
                        ...
                        <v-text-field dense outlined maxlength="30" v-model="tempSubcategory.name" :rules="nameRules"
                            ...
                            <v-list dense height="150" max-height="150" class="overflow-y-auto">
                                <v-list-item-group v-model="selectedMatchText" ripple="false">
                                    <v-list-item v-for="(matchText, idx) in tempSubcategory.matchText" :key="idx">
                                        ...
                                        <v-list-item-action>
                                            <v-btn small icon @click.stop="deleteMatchText(matchText, idx)"
                                                   color="primary"
                        ...
                        <div class="new-text-mapping-container">
                            ...
                            <v-text-field dense outlined v-model="newMatchText" label="New Match Text">
                                <template v-slot:append-outer>
                                    <v-btn small dark @click.stop="addMatchText" color="primary" title="Add Match Text"
                                           class="icon-btn">
    ...
    export default {
        ...
        methods: {
                ...
                if (this.isSubcategoryUnique(this.tempSubcategory.name)) {
                    ...
                    if (index !== -1) {
                        ...
                        CategoryService.updateCategory(this.tempCategory).then(() => {
                            // Emit the category-updated event
            ...
            addMatchText() {
                ...
                const idx = this.tempSubcategory.matchText.findIndex((match) => match.toLowerCase() === this.newMatchText.toLowerCase())
                if (idx === -1) {
